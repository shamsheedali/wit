...
const PlayWithBot: React.FC<{
  ...
  // Modify your doMove function in play-with-bot.tsx:

const doMove = useCallback((fen: engine.Fen, from: engine.Square, to: engine.Square) => {
...
useEffect(() => {
  ...
  return (
    <div className={styles.App}>
      <header><h1>♛ Chessbot</h1></header>
      <div className={styles.TopNav}>
        <BotSelector playerName="White" availableBots={bots} selectedBot={whiteBot} setSelectedBot={setWhiteBot} disabled={isPlaying} />
        <BotSelector playerName="Black" availableBots={bots} selectedBot={blackBot} setSelectedBot={setBlackBot} disabled={isPlaying} />
        <button className={styles.Button} onClick={() => setPlaying(!isPlaying)}>{isPlaying ? "Pause" : "Play"}</button>
        <button className={styles.Button} onClick={newGame}>Reset</button>
      </div>
      <Chessboard position={fen} onPieceDrop={(from, to) => doMove(fen, from, to)} boardWidth={400} />
    </div>
  );
};
